...
- (void)showWithStatus:(NSString*)string maskType:(SVProgressHUDMaskType)hudMaskType networkIndicator:(BOOL)show tishi:(NSString *)tishi;
...
@synthesize overlayWindow, hudView, maskType, fadeOutTimer, stringLabel, imageView, spinnerView, visibleKeyboardHeight,tishiLabel;
...
+(void)showWithStatusGra:(NSString *)status maskType:(SVProgressHUDMaskType)maskType tishi:(NSString *)tishi
{
    [[SVProgressHUD sharedView] showWithStatus:status maskType:maskType networkIndicator:NO tishi:tishi];
}
...
- (void)tishiString:(NSString *)string :(NSString *)stringL{
    ...
    if(string) {
        ...
        if(hudHeight > 100) {
            labelRect = CGRectMake(12, 5, hudWidth, stringHeight);
            labelRect1 = CGRectMake(12, 70, hudWidth, stringHeight);
            hudWidth+=24;
        } else {
            hudWidth+=24;
            labelRect = CGRectMake(0, 5, hudWidth, stringHeight);
            labelRect1 = CGRectMake(0, 70, hudWidth, stringHeight);
        }
    }
    ...
    self.tishiLabel.frame = labelRect;
    
    self.stringLabel.hidden = NO;
    self.stringLabel.text = stringL;
    self.stringLabel.frame = labelRect1;
    
    if(string)
        self.spinnerView.center = CGPointMake(ceil(CGRectGetWidth(self.hudView.bounds)/2)+0.5, 50.5);
    else
...
-(void)showWithStatus:(NSString *)string maskType:(SVProgressHUDMaskType)hudMaskType networkIndicator:(BOOL)show tishi:(NSString *)tishi
{
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        if (tishiLabel == nil) {
            tishiLabel = [[UILabel alloc] initWithFrame:CGRectZero];
            tishiLabel.textColor = [UIColor whiteColor];
            tishiLabel.backgroundColor = [UIColor clearColor];
            tishiLabel.adjustsFontSizeToFitWidth = YES;
            tishiLabel.textAlignment = NSTextAlignmentCenter;
            tishiLabel.baselineAdjustment = UIBaselineAdjustmentAlignCenters;
            tishiLabel.font = [UIFont boldSystemFontOfSize:16];
            tishiLabel.shadowColor = [UIColor blackColor];
            tishiLabel.shadowOffset = CGSizeMake(0, -1);
            tishiLabel.numberOfLines = 0;
        }
        
        if(!tishiLabel.superview)
            [self.hudView addSubview:tishiLabel];
        
        tishiLabel.text = tishi;
        tishiLabel.frame = CGRectMake(0, 0, self.frame.size.width, 20);
        tishiLabel.font = [UIFont boldSystemFontOfSize:14];
        tishiLabel.textColor = [UIColor whiteColor];
        tishiLabel.textAlignment = NSTextAlignmentCenter;
