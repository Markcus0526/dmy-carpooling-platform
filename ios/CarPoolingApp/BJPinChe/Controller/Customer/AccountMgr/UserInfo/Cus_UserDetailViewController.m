 ...
 */
#import "Cus_UserDetailViewController.h"
#import "RDActionSheet.h"
#import "NonRotateImagePickerController.h"
#import "ImageHelper.h"


@interface Cus_UserDetailViewController ()
...
@implementation Cus_UserDetailViewController
...
//	{
//		[self.view makeToast:@"手机号不能为空" duration:2 position:@"center"];
...
- (IBAction)onClickedConfirm:(id)sender
{
	...
	if ([[_txtOldPW text] isEqualToString:@""] == YES) {
		[self.view makeToast:@"原密码未填写" duration:2 position:@"center"];
	...
	if ([[_txtNewPW text] isEqualToString:@""] == YES) {
		...
		[self.view makeToast:@"新密码未填写" duration:2 position:@"center"];
		return;
	}

	if ([[_txtConfirmPW text] isEqualToString:@""] == YES) {
		[_txtConfirmPW becomeFirstResponder];
		[self.view makeToast:@"再次密码未填写" duration:2 position:@"center"];
		return;
	}
...
- (void) callChangeUserInfo
{
	...
	// Call the GetCurOrderList service routine.
    [[CommManager getCommMgr] accountSvcMgr].delegate = self;
    [[[CommManager getCommMgr] accountSvcMgr] ChangeUserInfo:[Common getUserID] mobile:[_txtPhoneNum text] nickname:[_txtName text] birthday:[_lblBirthday text] sex:[NSString stringWithFormat:@"%d", sex] photo:base64Image photo_changed:photoState carimg:@"" carimg_changed:0 devtoken:[Common getDeviceMacAddress]];
}
...
- (void) callChangePassword
{
	if (_txtOldPW.text.length < 6) {
		[self.view makeToast:@"原密码长度必须要6-16位之间" duration:2 position:@"center"];
		[_txtOldPW becomeFirstResponder];
		return;
	}

	if (_txtNewPW.text.length < 6) {
		[self.view makeToast:@"新密码长度必须要6-16位之间" duration:2 position:@"center"];
		[_txtNewPW becomeFirstResponder];
		return;
	}

	if (_txtConfirmPW.text.length < 6) {
		[self.view makeToast:@"再次密码长度必须要6-16位之间" duration:2 position:@"center"];
		[_txtConfirmPW becomeFirstResponder];
		return;
	}
