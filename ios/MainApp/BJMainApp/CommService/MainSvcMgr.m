...
- (void) GetChlidAppList : (NSString *)imei
{
	...
	NSDictionary *params = [NSDictionary dictionaryWithObjectsAndKeys:
                            ...
                            nil];
    //responseStr	__NSCFString *	@"{"result":{"retcode":0,"retmsg":"操作成功","retdata":[{"uid":47,"name":"3453454534","code":"gfdds","image":"","packname":"34525234534","urlscheme":"","latestver":1,"curver":"","curver_code":0,"downloadurl":"http:\/\/sypic.oicp.net:10100\/img\/upload\\gfdds\\45\\Firefox-setup29.0.1.5239.1399887243.exe"}]}}"	0x150ce200
    ...
    [httpClient getPath:method parameters:params success:^(AFHTTPRequestOperation *operation, id responseObject)
     {
		 ...
		 MyLog(@"GetChlidAppList Request Successful, response '%@'", responseStr);
     } failure:^(AFHTTPRequestOperation *operation, NSError *error) {
		 [delegate getChildAppListResult:SVCERR_MSG_ERROR dataList:nil];
		 MyLog(@"GetChlidAppList [HTTPClient Error]: %@", error.localizedDescription);
     }];
}
    ...
    [httpClient getPath:method parameters:params success:^(AFHTTPRequestOperation *operation, id responseObject)
     {
		 ...
		 MyLog(@" GetAnnouncePage Request Successful, response '%@'", responseStr);
     } failure:^(AFHTTPRequestOperation *operation, NSError *error) {
		 [delegate getAnnouncePageResult:SVCERR_MSG_ERROR dataList:nil];
		 MyLog(@"GetAnnouncePage [HTTPClient Error]: %@", error.localizedDescription);
     }];
}
    ...
    [httpClient getPath:method parameters:params success:^(AFHTTPRequestOperation *operation, id responseObject)
     {
		 ...
		 MyLog(@"GetLatestOrderInfos Request Successful, response '%@'", responseStr);
     } failure:^(AFHTTPRequestOperation *operation, NSError *error) {
		 [delegate getLatestOrderinfosResult:SVCERR_MSG_ERROR dataList:nil];
		 MyLog(@"GetLatestOrderInfos [HTTPClient Error]: %@", error.localizedDescription);
     }];
}
	...
	[httpClient getPath:@"readPersonNotifs" parameters:params success:^(AFHTTPRequestOperation *operation, id responseObject)
	 {
		 if (delegate != nil && [delegate respondsToSelector:@selector(readPersonNofityResultCode:retmsg:uid:)])
		 {
			 ...
			 if (nRetCode == SVCERR_SUCCESS)
			 {
				 [delegate readPersonNofityResultCode:nRetCode retmsg:MSG_SUCCESS uid:personnotifid];
			 }
			 else
			 {
				 [delegate readPersonNofityResultCode:nRetCode retmsg:szRetMsg uid:personnotifid];
			 }
		 }
	 } failure:^(AFHTTPRequestOperation *operation, NSError *error) {
		 [delegate readPersonNofityResultCode:SVCERR_FAILURE retmsg:SVCERR_MSG_ERROR uid:personnotifid];
		 NSLog(@"[HTTPClient Error]: %@", error.localizedDescription);
	 }];
}
    ...
    if (responseStr)
    {
        ...
        else
        {
            ...
            for (NSDictionary * item in jsonArray) {
                ...
                stInfo.packname = [item objectForKey:@"packname"];
