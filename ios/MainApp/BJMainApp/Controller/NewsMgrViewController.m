...
@interface NewsMgrViewController ()
...
@implementation NewsMgrViewController
...
- (void) initControls
{
	...
	announcement_count = [Config unreadNews1Count];
	ordernotify_count = [Config unreadNews2Count];
	personnotify_count = [Config unreadNews3Count];
...
- (void)cellClickedDelegate:(long)uid withType:(int)news_type
{
	...
	// save unread news count
	[Config setUnreadNewsCount:announcement_count news2:ordernotify_count news3:personnotify_count];
...
- (void) readPersonNofityResultCode:(int)retcode retmsg:(NSString *)retmsg uid:(long)personnotifid
{
	...
	for (int i = 0; i < mNewsArray3.count; i++)
	{
		STNewsInfo* arrayItem = [mNewsArray3 objectAtIndex:i];
		
		if (arrayItem.uid == personnotifid)
		{
			newsItem = arrayItem;
			nIndex = i;
