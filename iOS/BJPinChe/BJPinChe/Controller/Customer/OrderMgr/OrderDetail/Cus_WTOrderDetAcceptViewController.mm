//
//  Cus_WTOrderDetAcceptViewController.m
//  BJPinChe
//
...
@interface Cus_WTOrderDetAcceptViewController ()

@end

@implementation Cus_WTOrderDetAcceptViewController
...
{
    [self callRefuseOrder];
}

#pragma Set Password Result
- (void) tappedDone:(NSString *)result
{
    ...
    // call accept order using set password
    [self callAcceptOrder:result];
}
...
- (void) callAcceptOrder : (NSString *)password
{
	TEST_NETWORK_RETURN
	
    [SVProgressHUD show];
    [[CommManager getCommMgr] orderSvcMgr].delegate = self;
    [[[CommManager getCommMgr] orderSvcMgr] ConfirmOnOffOrder:[Common getUserID] OrderId:self.mOrderInfo.uid Password:password DevToken:[Common getDeviceMacAddress]];
}
 
- (void) callRefuseOrder
{
	TEST_NETWORK_RETURN
	
    [SVProgressHUD show];
    [[CommManager getCommMgr] orderSvcMgr].delegate = self;
    [[[CommManager getCommMgr] orderSvcMgr] RefuseOnOffOrder:[Common getUserID] OrderId:self.mOrderInfo.uid DevToken:[Common getDeviceMacAddress]];
}
