...
- (void)setupRefresh
{
	...
	[_mainTable addHeaderWithTarget:self action:@selector(headerRefreshing1)];
	// 2.上拉加载更多(进入刷新状态就会调用self的footerRereshing)
	[_mainTable addFooterWithTarget:self action:@selector(footerRefreshing1)];
...
{
	if ((mCouponArray == nil) || ([mCouponArray count] == 0))
	{
		mCurPageNum = 0;
		[self callGetCouponPage:mCurPageNum];
	}
	...
	{
		[self callGetLatestCoupon:[(STCouponInfo*)[mCouponArray objectAtIndex:0] uid]];
	}
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    MyCouponTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:@"couponcell"];
    if (cell == nil)
	{
		cell = (MyCouponTableViewCell *)[MyCouponTableViewCell cellFromNibNamed:@"MyCouponTableViewCell"];
	}
    cell.selectionStyle = UITableViewCellSelectionStyleNone;
    cell.delegate = self;
    
	[cell initWithData:(id)data reuseIdentifier:@"couponcell" parent:self dispno:indexPath.row + 1];
