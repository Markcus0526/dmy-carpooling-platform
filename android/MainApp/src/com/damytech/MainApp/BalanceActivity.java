...
public class BalanceActivity extends SuperActivity
{
		...
		if (!Global.isPersonVerified(getApplicationContext()))
		{
			CommonAlertDialog dialog = new CommonAlertDialog.Builder(BalanceActivity.this)
					...
					.positiveListener(new View.OnClickListener() {
						@Override
						public void onClick(View v) {
							Intent intent = new Intent(BalanceActivity.this, VerifyActivity.class);
							intent.putExtra(Global.ANIM_DIRECTION(), Global.ANIM_COVER_FROM_RIGHT());
							BalanceActivity.this.getIntent().putExtra(Global.ANIM_DIRECTION(), Global.ANIM_COVER_FROM_LEFT());
							startActivity(intent);
						}
	...
	private void onClickWithDraw()
	{
		...
		{
			Global.showAdvancedToast(BalanceActivity.this,
					getResources().getString(R.string.STR_BALANCE_PERSON_NOTVERIFY),
					Gravity.CENTER);
			return;
		}
		...
		try {
			...
			startProgress();
			CommManager.withdraw(Global.loadUserID(getApplicationContext()), szRealname, szBankcard, balance, edt_login_pwd.getText().toString(), Global.getIMEI(getApplicationContext()), withdraw_handler);
		} catch (Exception ex) {
			ex.printStackTrace();
		}
	}
	...
	private AsyncHttpResponseHandler withdraw_handler = new AsyncHttpResponseHandler()
	{
		...
		public void onSuccess(String content) {
			...
			try {
				...
				if (nRetcode == 0)
				{
					Global.showAdvancedToast(BalanceActivity.this, getResources().getString(R.string.STR_BALANCE_WITHDRAW_SUCCESS), Gravity.CENTER);
					...
					String strText = getString(R.string.STR_BALANCE_MYBALANCE) + "  " + Double.toString(last_balance);
					SpannableString ss = new SpannableString(strText);
					ss.setSpan(new ForegroundColorSpan(getResources().getColor(R.color.LIGHT_YELLOW_COLOR)), 6, 6 + Double.toString(cur_balance).length(), 0);
					lblRemain.setText(ss);
		...
		public void onSuccess(int statusCode, String content) {
			...
			try {
				...
				if (nRetcode == 0)
				{
					...
					String strText = getString(R.string.STR_BALANCE_MYBALANCE) + "  " + Double.toString(money);
					SpannableString ss = new SpannableString(strText);
