...
public class STDrvOnceOrder
{
	...
	public static STDrvOnceOrder decodeFromJSON(JSONObject jsonObj)
	{
		STDrvOnceOrder orderInfo = new STDrvOnceOrder();
		...
		try { orderInfo.distance = jsonObj.getDouble("distance"); } catch (Exception ex) { ex.printStackTrace(); }
		try { orderInfo.mileage = jsonObj.getDouble("mileage"); } catch (Exception ex) { ex.printStackTrace(); }
		try { orderInfo.distance_desc = jsonObj.getString("distance_desc"); } catch (Exception ex) { ex.printStackTrace(); }
		...
		try { orderInfo.status = jsonObj.getString("status"); } catch (Exception ex) { ex.printStackTrace(); }
        ...
        try { orderInfo.insuranceFee = jsonObj.getDouble("insu_fee"); } catch (Exception ex) { ex.printStackTrace(); }
