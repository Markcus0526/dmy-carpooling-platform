...
*/
	private void onClickLogin()
	{
        ...
        {
            Global.showAdvancedToast(LoginActivity.this, getResources().getString(R.string.STR_NO_NETWORK), Gravity.CENTER);
            return;
        }
        ...
        if (edt_username.getText().toString().trim().length() != 11)
        {
            Global.showAdvancedToast(LoginActivity.this, getResources().getString(R.string.STR_LOGIN_USERNAME_CANNOT_LENGTH), Gravity.CENTER);
            edt_username.requestFocus();
            edt_username.selectAll();
            return;
        }
		...
		startProgress();
		CommManager.loginUser(edt_username.getText().toString(),
	...
	private AsyncHttpResponseHandler login_handler = new AsyncHttpResponseHandler()
	{
		...
		public void onSuccess(String content) {
            ...
            try
			{
				...
				if (nRetcode == ConstData.ERR_CODE_NONE)
				{
                    ...
                    double runDist = exec_info.getDouble("distance");
                    if (runOrderId > 0)
                    {
                        Intent intent = new Intent(LoginActivity.this, DrvCityOrderExeActivity.class);
                        intent.putExtra(Global.ANIM_DIRECTION(), Global.ANIM_COVER_FROM_RIGHT());
		...
		public void onFailure(Throwable error, String content) {
			...
			stopProgress();
			Global.showAdvancedToast(LoginActivity.this, getResources().getString(R.string.STR_CONN_ERROR), Gravity.CENTER);
		}
	};
