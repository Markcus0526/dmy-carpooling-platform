...
public class STDrvEvalInfo {
	...
	public static STDrvEvalInfo decodeFromJSON(JSONObject jsonObj)
	{
		STDrvEvalInfo evalInfo = new STDrvEvalInfo();

		try { evalInfo.uid = jsonObj.getLong("uid"); } catch (Exception ex) { ex.printStackTrace(); }
		try { evalInfo.pass_name = jsonObj.getString("driver_name"); } catch (Exception ex) { ex.printStackTrace(); }
