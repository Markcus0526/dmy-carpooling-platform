...
public class FeedbackActivity extends SuperActivity
{
	...
	private void onClickSend()
	{
        ...
        if (edt_contents.getText().toString().length() > 1000)
        {
            Global.showAdvancedToast(FeedbackActivity.this, getResources().getString(R.string.STR_FEEDBACK_ERROR_LENGTH), Gravity.CENTER);
            edt_contents.requestFocus();
            return;
        }

		startProgress();
		CommManager.advanceOpinion(Global.loadUserID(getApplicationContext()), edt_contents.getText().toString(), Global.getIMEI(getApplicationContext()), submit_handler);
	}
