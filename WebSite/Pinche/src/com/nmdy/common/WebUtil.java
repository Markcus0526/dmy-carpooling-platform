...
public class WebUtil {
    ...
    public static int _getLoggedUserId() {
    	UserLoginInfo info = (UserLoginInfo)WebUtil._session("user");
    	return info.getId();
    }
    
    public static String _getActionUrl() {
    	HttpServletRequest request = ServletActionContext.getRequest();
    	String fullPath = request.getRequestURI(); 
    	String relatedPath = fullPath.split(request.getContextPath())[1];
    	return relatedPath;
    }
    
    public static ServletOutputStream _getResponseOutputStream() throws IOException {
    	HttpServletResponse resp = ServletActionContext.getResponse();
