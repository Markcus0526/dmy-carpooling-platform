...
<script>
			...
			columns:[[
	             {field:'operMenu',width:100,align:'center',title:'操作',
            	  formatter:function(value,row,index){  
		               var btn="";
		              btn+= '<a href="javascript:void(0)" onclick="view('+row.id+','+row.order_type+')">查看</a>|';
		     		  btn+=' <a href="javascript:void(0)" onclick="edit('+row.id+','+row.order_type+')">修改</a>';
		               return btn;  
	        	  }	 
	             }, 
	             {field:'orderNum',width:150,align:'center',title:'订单编号'},
	             {field:'startCity',width:70,align:'center',title:'所在城市'},
	             ...
	             {field:'carPhone',width:150,align:'center',title:'车主手机'},
	             {field:'status',width:70,align:'center',title:'订单状态',
	              	 formatter:function(value,row,index){ 
	            		  if(value==1){
				        	  return '发布';
				          } if(value==2){
	             ...
	             {field:'statusTime',width:170,align:'center',title:'状态时间'},
	         ]],
	...
	function view(id,order_type) {
		window.location=page_path + 'view1.do?id='+ id+'&order_type='+order_type;
	}
	
	function edit(id,order_type) {
		window.location=page_path + "edit1.do?id="+id+'&order_type='+order_type;
	}
	function loaddone() {
	...
	<div class="easyui-panel" data-options="fit:true"
		...
		<div class="easyui-layout" data-options="fit:true,border:true"
			...
			<div data-options="region:'north',title:'',border:true"
				style="height: 200px;">
				<h2 style="margin-bottom: 0px">拼车订单管理</h2>
				<form id="searchForm" action="search">
					<input id="param" type="hidden" name="param" value="search" />
					<div style="padding: 5px; margin-left: 10px">
						...
						<input id="begintime" name="begintime" class="easyui-datetimebox"
							style="width: 145px" data-options="formatter:myformatter">~
						<input id="endtime" name="endtime" class="easyui-datetimebox"
							style="width: 145px" data-options="formatter:myformatter">
						<span style="margin-left: 50px">订单编号:</span> <input id="order_num"
							name="order_num" class="textbox _textbox">
					</div>
					<div style="padding: 5px; margin-left: 10px">
						订单状态: <input id="status_1" style="margin-left: 20px"
							checked="checked" type="radio" name="status" value="1"><label
							for="status_1">发布</label> <input id="status_2"
							style="margin-left: 20px" type="radio" name="status" value="2"><label
					...
					<div style="padding: 5px; margin-left: 10px">
						订单类型: <input id="orderType_1" style="margin-left: 30px"
							checked="checked" type="checkbox" name="order_type1" value="1"><label
							for="orderType_1">单次拼车</label> <input id="orderType_2"
							style="margin-left: 30px" checked="checked" type="checkbox"
							name="order_type2" value="2"><label for="orderType_2">上下班拼车</label>
						<input id="orderType_3" style="margin-left: 25px"
							checked="checked" type="checkbox" name="order_type3" value="3"><label
							for="orderType_3">长途拼车</label>
					</div>
