...
<form id="staticsForm" method="post" action="<%=basePath%>/spread/activity/activity_insertActivity.do">
	...
	<div style="padding: 10px 20px 0px 20px">
		<span>活动时限:</span>
		<input id="beginDate" name="activity.startTime" class="easyui-datetimebox" 
			style="width:145px;" data-options="formatter:mydateformatter"/>&nbsp;~
		<input id="endDate" name="activity.endTime" class="easyui-datetimebox" 
			style="width:145px;" data-options="formatter:mydateformatter"/>	
	</div>
	<div style="padding: 10px 20px 0px 20px">
		<span>分享有效时间(小时):</span>
		<input type="text" name="activity.shareTime" />
	</div>
	<div style="padding: 10px 20px 0px 20px">
		<span>分享有效人数:</span>
		<input type="text" name="activity.sharePeople" />
	</div>
	<div style="padding: 10px 20px 0px 20px">
		<span>奖品设置:</span>
		<input type="radio" name="activity.prizeType" value="0" checked="checked" id="aaa"/>点卷
		<input type="radio" name="activity.prizeType" value="1" id="bbb"/>第三方奖品
	</div>
	<div id="gift" style="display: none;">
		<div style="padding: 10px 20px 0px 20px">
			<span>奖品名称:</span>
			<input type="text" name="activity.lotteryName"/>
		</div>
		</div>
	...
	<div id="dianjuan">
		<div style="padding: 10px 20px 0px 20px">
<%-- 			<span>奖励对象:</span> --%>
<!-- 			<input type="text" name="activity.rewardStartPeople" />&nbsp;名~ -->
<!-- 			<input type="text" name="activity.rewardEndPeople" />&nbsp;&nbsp;&nbsp; -->
			<span>奖品设置：</span>
			<input type="button" value="选择点卷" onclick="showGift();"/>	
		</div>
		说明：第0名为活动发起人
			<div style="padding: 10px 20px 0px 20px">
			<span>已选点卷:</span>
			<input type="text" value="${sessionScope.giftNames}"/> 
			<input type="button" value="添加新规则" onclick="addRule();"/> 
		</div>
	...
	<td colspan="4" style="text-align:center">
     ...
     <div id="dlg" class="easyui-dialog"
		...
		<form id="fm" method="post" action="<%=basePath%>spread/activity/activity_choseGift.do">
		<input type="hidden" name="result" value="add" />
		<div data-options="region:'north'" style="width:55%">
			...
			<div style="position:relative;border:0px red solid;width:100%;height:100px;margin-left:auto;margin-right:auto;margin-top:5px">
				...
				<div style="position:absolute;top:25px;left:12%;border:0px red solid;width:60%;height:25px;">
					<input type="text" name="giftNames" class="easyui-validatebox textbox" style="width: 300%;height:20px" id="giftNames"></input>
					<input name="giftIds" id="giftIds" type="hidden" readonly="readonly"/>
				</div>
	...
	<div id="ruleDiv" class="easyui-dialog" style="width:800px; height:500px; padding: 10px 20px; top:30px" closed="true" buttons="#dlg-buttons">
		
		<form id="ruleFm" method="post" action="<%=basePath%>spread/activity/activity_insertRule.do">
			<div style="position:relative;border:0px red solid;width:100%;height:100px;margin-left:auto;margin-right:auto;margin-top:5px">
			<c:set value="${fn:split(sessionScope.giftNames,',')}" var="names"></c:set>
			
			<div style="padding: 10px 20px 0px 20px">
				<span>选择点卷:</span>
				<select name="ruleName">
						<c:if test="${!empty listGift}">
			...
			<div style="padding: 10px 20px 0px 20px">
				<span>开始人数:</span>
				<input type="text" name="ruleStartNum" />
			</div>

			<div style="padding: 10px 20px 0px 20px">
				<span>结束人数:</span>
				<input type="text" name="ruleEndNum" />
			</div>
...
function mydateformatter(date){
	...
	var staLoader = {};
		...
		dataLoader.load = function(){
			$('#loanList').datagrid({
				url: '<%=basePath%>/spread/activity/activity_listSessionActivityRole.do',
				loadMsg : '数据处理中，请稍后....',
				pagination: false,
				fitColumns : true,
				columns:[[
...
// 					$('#num').val(num);
		...
		function delRule(ruleName,startNum,endNum){
			var url = "<%=basePath%>/spread/activity/activity_removeRule.do?ruleName="+ruleName+"&ruleStartNum="+startNum+"&ruleEndNum="+endNum;
			location = url;
		}
</script>
</body>
