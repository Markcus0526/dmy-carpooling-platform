...
<form id="staticsForm" method="post" action="<%=basePath%>/spread/activity/activity_insertActivity.do">
	...
	<div id="dianjuan">
		...
		说明：第0名为活动发起人
			<div style="padding: 10px 20px 0px 20px">
			<span>已选点卷:</span>
			<input type="text" value="${giftNames}"/> 
		</div>
     ...
     <div id="dlg" class="easyui-dialog"
		...
		<input type="hidden" name="result" value="show" />
			...
			<div style="padding: 10px 20px 0px 20px">
				...
				<select name="ruleName">
						<c:forEach items="${listGift}" var="gift">
							<option value="${gift.name}">${gift.name}使用${gift.num}张</option>
						</c:forEach>
				</select>
			</div>
...
// 					$('#num').val(num);
		...
		function delRule(ruleName,startNum,endNum){
			var url = "<%=basePath%>/spread/activity/activity_removeRule.do?ruleName="+ruleName+"&startNum="+startNum+"&endNum="+endNum;
			alert(url);
		}
</script>
</body>
