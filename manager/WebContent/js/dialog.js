var IMAGESPATH = "http://localhost:8080/bk/images/dialog/";
...
Dialog.prototype.displacePath = function() {
	if (this.URL.substr(0, 4) == "http" || this.URL.substr(0, 1) == "/" || this.URL.substr(0, 11) == "javascript:") {
		return this.URL
	} else {
		var a = this.URL;
...
Dialog.setSize = function(w,h) {
	if (topWin.Dialog._dialogArray.length > 0) {
		...
		if (w && +w > 20) {
			a.Width = +w;
			topWin.$id("_DialogTable_" + a.ID).width = a.Width + 26;
			topWin.$id("_Container_" + a.ID).style.width = a.Width + "px"
		}
		if (h && +h > 10) {
			a.Height = +h;
			topWin.$id("_Container_" + a.ID).style.height = a.Height + "px"
		}
		a.setPosition();
	}
};
...
Dialog.setTitle = function(str) {
	if (topWin.Dialog._dialogArray.length > 0) {
		var a = topWin.Dialog._dialogArray[topWin.Dialog._dialogArray.length - 1];
		topWin.$id("_Title_" + a.ID).innerHTML =str;
	}
};
Dialog.showImeButton = function(b) {
	if(b){
		var a = topWin.Dialog._dialogArray[topWin.Dialog._dialogArray.length - 1];
		var d = topWin.$id("_ButtonIme_" + a.ID);
		if (d) {
...
//有确定按钮的HTML弹窗
Dialog.validator = function(t, f, c, a, b) {
	var a = a || 300,
	b = b || 110;
	var d = new Dialog({
		Width: a,
	...
	d.InnerHtml = '<table height="100%" border="0" align="center" cellpadding="10" cellspacing="0">		<tr><td align="left" id="Message_' + this.ID + '" style="font-size:9pt">' + f + "</td></tr>	</table>";
	d.show();
	d.okButton.parentNode.style.textAlign = "center";
	d.okButton.style.display = "none";
	d.cancelButton.value = d.OkButtonText
